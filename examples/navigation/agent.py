...
def evaluate(eval_policy: TD3, env_config: dict, restriction_violation_fns, evaluation_envs: int = 20):
    ...
    for it in range(evaluation_envs):
        ...
        for eval_agent in eval_env.agent_iter():
            ...
            if term or trunc:
                eval_action = None
                if eval_agent == 'agent_0':
                    metrics['solved'].append(inf['solved'])
            elif eval_agent == 'agent_0':
    ...
    return sum(metrics['rewards']) / len(metrics['rewards']), \
           sum(metrics['solved']) / len(metrics['solved']), \
           sum(metrics['steps']) / len(metrics['steps'])
